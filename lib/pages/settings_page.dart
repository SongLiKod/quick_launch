...
/// 设置正文内容，可在 SettingsPage 或侧边抽屉中复用
  ...
  return ValueListenableBuilder<int?>(
    ...
    builder: (_, mods, _) {
      ...
      return ListTile(
        ...
        subtitle: Text(label == '未设置' ? '未设置' : '按 $label 显示主窗口'),
        trailing: TextButton(
        ...
        TextButton(
          onPressed: () {
            ...
            ScaffoldMessenger.of(
              context,
            ).showSnackBar(const SnackBar(content: Text('已清除显示窗口快捷键')));
          },
  ...
  return ValueListenableBuilder<int?>(
    ...
    builder: (_, mods, _) {
      ...
      return ListTile(
        ...
        subtitle: Text(label == '未设置' ? '未设置' : '按 $label 打开全局搜索'),
        trailing: TextButton(
      ...
      actions: [
        ...
        TextButton(
          onPressed: () {
            ...
            ScaffoldMessenger.of(
              context,
            ).showSnackBar(const SnackBar(content: Text('已清除全局搜索快捷键')));
          },
          child: const Text('清除', style: TextStyle(color: Colors.red)),
        ),
        FilledButton(
...
Future<void> _importConfig(BuildContext context) async {
  ...
  try {
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(SnackBar(content: Text('成功导入 $count 个启动项')));
  } catch (e) {
    if (!context.mounted) return;
    ScaffoldMessenger.of(
      context,
...
Widget _themeTile(BuildContext context, SettingsService service) {
  ...
  return ValueListenableBuilder<ThemeMode>(
    ...
    builder: (_, mode, _) => ListTile(
      ...
      subtitle: Text(['浅色', '深色', '跟随系统'][modes.indexOf(mode)]),
...
Widget _customIconTile(BuildContext context, SettingsService service) {
  return ValueListenableBuilder<String?>(
    ...
    builder: (_, iconPath, _) => ListTile(
      ...
      subtitle: Text(iconPath != null ? '已自定义' : '使用默认图标'),
      trailing: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          if (iconPath != null)
...
Widget _columnCountTile(BuildContext context, SettingsService service) {
  return ValueListenableBuilder<int>(
    ...
    builder: (_, count, _) => ListTile(
      ...
      subtitle: Text(count <= 1 ? '单列' : '$count 列'),
      trailing: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          IconButton(
          ...
          Text('列', style: TextStyle(fontSize: 12, color: Colors.grey[600])),
          IconButton(
...
void _pickIcon(BuildContext context, SettingsService service) async {
  ...
  try {
    ...
    if (ok != true) {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('图标文件无效，请选择其他 .ico 文件')));
      return;
    }
  } catch (e) {
    if (!context.mounted) return;
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(SnackBar(content: Text('设置图标失败: $e')));
    return;
  }
  ...
  ScaffoldMessenger.of(
    context,
  ).showSnackBar(const SnackBar(content: Text('图标已更新')));
}
...
void _resetIcon(BuildContext context, SettingsService service) async {
  ...
  ScaffoldMessenger.of(
    context,
  ).showSnackBar(const SnackBar(content: Text('已恢复默认图标')));
}
